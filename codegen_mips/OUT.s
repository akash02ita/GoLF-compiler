...
main:
	...
	sw $ra, 0($sp)
	addi $fp, $sp, 8
	# Alloc itworks_file_main
	addi $sp, $sp, -4  # add memory for lv
	sw $zero, 0($sp)
	li $t0, 1234
	sw $t0, -12($fp) # assignment for itworks int
	lw $t0, -12($fp) # load content of itworks int
...
.data
.align 2
string_3: .byte  10, 10, 0
.text
	la $t0, string_3 # loading address of 1st char of string `\n\n`
	# Adding arg #1 in functioncall prints()
	addi $sp, $sp, -4
	sw $t0, 0($sp)
	jal prints
